...
class IndexPage(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        promote_data = []
        all_promote_article = Article.objects.filter(promote=True)[:3]
        ...
        context = {
            'all_article_objs': all_article_objs,
            'promoted_data': promote_data,
        }
...
class SingleArticleAPIView(APIView):
    ...
    def get(self,request,format=None):
        try:
            ...
            return Response({'status':"Problem on the server!!!"},
...
class UpdateArticleAPIView(CreateAPIView):
    def post(self,request,format=None):
        try:
            ...
            Response({'status':'Intenal Server Error'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
